...
import { Button, StyleSheet, Text, TouchableOpacity, View, ImageBackground, Image, Dimensions  } from 'react-native';
...
ntc.init();
    ...
    const maskColWidth = (width - 100) / 2;
    if (!permission) {
        return <View />;
    }
    ...
    }  let getResizePercentage = (width, height) => {
        let percentage = 0
        let newWidth = width
        let newHeight = height

        while (newWidth > 1400 || newHeight > 1350) {
            percentage += 0.01
    ...
    let takePicture = async () => {
        if (this.camera) {
            ...
            let percentage = getResizePercentage(photo.width, photo.height)
            console.log(percentage)
            const manipulateResult = await manipulateAsync(
                photo.uri,
                [{crop: {
                        originX: photo.width*0.35,
                        originY: photo.height*0.35,
                        height: photo.height*0.3,
                        width: photo.width*0.3
                    }}],
                {}
            );
            ImageColors.getColors(manipulateResult.uri,  {
                fallback: '#228B22',
            }).then((color) => {
              try{
                let colorName = color.vibrant.replace('#', '')

                let newName = ntc.name(color.vibrant)
                console.log(newName)
                colorName = GetColorName(colorName)
                let colorBrightness = getBridnis(color.vibrant)
                alert('Color name: ' + newName[3] + '\n' + 'Color code: ' + color.vibrant + '\n' + 'Color brightness: ' + colorBrightness )
                setColors(color)
              }catch (e) {
        ...
        <View style={styles.container}>
            {image ? (
                <View style={{width:'100%',height:'100%'}}>
                    <Text onPress={clearImage} style={styles.clearIcon}>X</Text>
